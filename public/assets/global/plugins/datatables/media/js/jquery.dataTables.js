...
(/** @lends <global> */function( $ ) {
	 ...
	 *    if nTr is.
	 *  @returns {int} >=0 if successful (index of new aoData entries), -1 if failed
	 *  @memberof DataTable#oApi
	 */
				...
				if ( src !== "" )
				{
					...
					for ( var i=0, iLen=a.length ; i<iLen ; i++ )
					{
						...
						if ( arrayNotation )
						{
							...
							// Traverse each entries in the array getting the properties requested
			...
			var setData = function (data, val, src) {
				...
				for ( var i=0, iLen=a.length-1 ; i<iLen ; i++ )
				{
					...
					if ( arrayNotation )
					{
						...
						// Traverse each entries in the array setting the properties requested
	...
	function _fnBindAction( n, oData, fn )
	{
		$(n)
			.bind( 'click.DT', oData, function (e) {
					n.blur(); // Remove focus outline for mouse clients
					fn(e);
				} )
		...
		iterator: function ( flatten, type, fn ) {
			...
			for ( i=0, ien=context.length ; i<ien ; i++ ) {
				if ( type === 'table' ) {
					...
					// this has same length as context - one entries for each table
	...
	DataTable.defaults = {
		 ...
		 * define your columns, you must have an entries in the array for every single
	...
	DataTable.ext = _ext = {
		 ...
		 * 
		 * Each entries in this object is a function and defines which buttons should
