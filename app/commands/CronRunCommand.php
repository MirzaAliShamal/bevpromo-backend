...
class CronRunCommand extends Command {
	...
	public function fire()
	{
		...
		$this->dailyAt('02:30',function()
		{
			// In the function, you can use anything that you can use everywhere else in Laravel.
            // Like models:
            $today = date('Y-m-d');
            $coupons = Coupon::where('expires','<',$today)->update(array('active' => false)); // Not really useful, but possible
			// You can append messages to the cron log like so:
			$this->messages[] = $coupons . ' Coupons Rows Effected';
		});
		$this->finish();
	}
